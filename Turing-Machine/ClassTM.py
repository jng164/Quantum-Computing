...
class TuringMachine:
    ...
    def print_tapes(self):
        print(f"Input Tape:  {self.i_tape}")
        print(f"Working tape: {self.w_tape}")
        print(f"Track Tape: {self.t_tape}")
        print(f"Output Tape: {self.o_tape}")



    def print_solution(self):
    # Remove 'S' and 'E' from the string
        final_tape = self.o_tape[3:-3]
    # Split the string into a list of binary numbers
        final_tape = final_tape.split('_')
        final_tape = [int(binary, 2) for binary in final_tape if binary]
        print(f"Ouput Tape: {final_tape}")
